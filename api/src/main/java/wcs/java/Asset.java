...
public class Asset extends AssetBase implements wcs.api.Asset, wcs.api.Content {
    ...
    public String getLocale() {
        ...
        try {
            ...
            ad = adm.readAttributes(new AssetIdImpl(c, cid), Arrays.asList("Dimension"));
            if (ad != null) {
                HashSet set = (HashSet) ad.getAttributeData("Dimension").getData();
                DimensionAssetInstance dim = null;
                for (Object o : set) {
                    dim = (DimensionAssetInstance) o;
                }
                return dim.getName();
            }
        } catch (Exception ex) {
            log.error(ex, "exception in getLocale");
        }
        return null;
    }
